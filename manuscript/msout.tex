...
%DIF ADD phylogenetic_signal_manuscript.tex   Mon Dec  9 11:13:05 2019
\usepackage{amsmath}
\usepackage{url}
...
%DIFAUXCMD
\DIFdelend %DIF >  \section*{Word Counts}

\DIFdelbegin \DIFdel{Main text: 5612 %DIF <  Limit 6,500
}\DIFdelend %DIF >  Main text: 6498 % Limit 6,500 % Can also subtract 25 for page numbers
...
%DIF >    \item Introduction: 1165 
%DIF >    \item Materials and Methods: 2470
%DIF >    \item Results: 960
%DIF >    \item Discussion: 1820
%DIF >    \item Acknowledgements: 83
%DIF >  \end{itemize}
...
\DIFdelbegin \DIFdel{Figures: 3
}\DIFdelend %DIF >  \noindent Tables: 2


\DIFdelbegin \DIFdel{Tables: 2
}\DIFdelend %DIF >  \noindent Supporting information: 1 file containing 5 sections.


\DIFdelbegin \DIFdel{Supporting information: 1 file containing 4 sections.
}\DIFdelend %DIF >  \vspace{0.2 in}

\DIFdelbegin %DIFDELCMD < \vspace{0.4 in}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section*{Summary} %DIF <  Must be <200 words. Currently 199
...
\citep{Mayr2001,Sauve2016}}\hspace{0pt}%DIFAUXCMD
...
\citep{Rezende2007a,Gomez2010,Rohr2014a,Fontaine2015,Lind2015,Ibanez2016,Bergamini2017,Sydenham2017,Volf2017,Hutchinson2017}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend .
...
\citep{Strauss2002,Adler2004,Strauss2006,Theis2006}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 
...
\section*{Materials and Methods} 
  ...
  \subsection*{Network data}
    ...
    Specifically, we removed }\DIFdelend \DIFaddbegin \DIFadd{traits and therefore cannot be expected to show the same trends
...
\DIFdel{for species }\DIFdelend \DIFaddbegin \DIFadd{$U_{ij}$ the set of unshared interaction partners for plants }\DIFaddend $i$ and $j$\DIFdelbegin \DIFdel{respectively.
    ...
    }\DIFaddend Note that species sharing a large \emph{number} of interaction partners may not share a large \emph{proportion} of interaction partners if the number of interaction partners that are not shared is also large. % Added because Josh got confused about the distinction.
\DIFdelbegin \DIFdel{To capture all of this information for each species pair, we therefore recorded the number of 
  ...
  \subsection*{\DIFdelbegin \DIFdel{Statistical analysis}\DIFdelend \DIFaddbegin \DIFadd{Testing conservation of niche overlap within networks}\DIFaddend } 
    ...
    $j$ \DIFaddbegin \DIFadd{and $J_{ij}$ is defined by the tuple ($M_{ij}$, $U_{ij}$) (see }\emph{\DIFadd{Supplemental Information 2}} \DIFadd{for R implementation)}\DIFaddend . 
...
base function ``glm".
    ...
    \subsubsection*{\DIFadd{Accounting for non-independence}}
      ...
      alternative hypothesis was set farther from zero \DIFaddbegin \DIFadd{(}\emph{\DIFadd{Supporting Information 3}}\DIFadd{)}\DIFaddend .
...
\DIFdelend % To determine how overlap of interaction partners
  ...
  \subsection*{Linking network-level trends and community composition}
    ...
    \DIFdelbegin \DIFdel{in each
    network }\DIFdelend using a non-parametric permutational multi-variate 
    analysis of variance (PERMANOVA;~\citealp{Anderson2001}).
    \DIFdelbegin \DIFdel{As we did not wish to inflate }\DIFdelend \DIFaddbegin \DIFadd{To avoid inflating }\DIFaddend the perceived similarity between
...
(}\emph{\DIFadd{Supplemental Information 4}}\DIFadd{).
}\DIFaddend 
...
\DIFdelbegin \DIFdel{As extreme specialists by definition 
    ...
    permuted networks, there was no relationship, which speaks to the robustness of our methodology (\emph{Supporting information \DIFdelbegin \DIFdel{3}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend }). 
...
, all of these slopes remain significant (}\emph{\DIFadd{Supplemental Information 4}}\DIFadd{).
    }\DIFaddend There was no significant relationship between overlap and phylogenetic distance in
    a further 34 plant families (see \emph{Supporting information \DIFdelbegin \DIFdel{4}\DIFdelend \DIFaddbegin \DIFadd{5}\DIFaddend } for further 
    ...
    increasing phylogenetic distance. \DIFaddbegin \DIFadd{If we again apply the correlated Bonferroni correction, all five significant slopes remain significant  (}\emph{\DIFadd{Supplemental Information 4}}\DIFadd{).
}\DIFaddend 
...
\citep{Novotny2005,Brandle2006,Astegiano2017}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
