...
{\Large \bf Reply to Referee 2}
	...
	The Referee seems to view our basic question and analyses positively, but is critical of our writing in terms of logic, clarity, and style. We were confused by some of these criticisms as there are points where the Referee asks why we did not conduct analyses that we did in fact perform or asks that we address biological complications that already appear later in the text. In these cases, we point to existing evidence in the manuscript and have tried to make these connections more obvious. We address the Referee's other comments in detail below (preceded by \textbf{R:}). We hope that this response will clarify the focus and scope of our analyses.
	...
	1. General criticism of the language of the text
		...
		\textbf{R:} We acknowledge that our choice of vocabulary or phrasing may have obfuscated some of the finer points of the manuscript. As such, we have endeavoured to clarify the manuscript as the Referee suggests.
	...
	2. Title lacks clarity
		...
		\textbf{R:} We have revised the title following a more detailed suggestion by Referee 3 and believe that the new title provides additional logical consistency.
	...
	5. Very confusing comment referencing paragraph 4

		\begin{refquote}
		4th paragraph "Considering factors affecting the dominant plant families within
		...
		\textbf{R:} The Referee appears to object to any reference to plant families, but has not made the reasons for this objection clear. As noted above, we do in fact consider relationships between phylogenetic distance and interaction partner overlap at \emph{all} taxonomic levels, not just within families. Apart from this, we are confused by the remainder of the Referee's comment. By 'dominant', we mean 'most species-rich in the focal community' rather than any kind of functional interpretation. Note that we do not address functions in the rest of the manuscript, so it is not clear why the Referee makes this leap. In order to avoid future confusion, we now replace 'dominant families' with 'species-rich groups'. We hope that this makes it clear that we are advocating research that takes a finer-scale perspective on phylogenetic signal in ecological networks.
	...
	6. Comment about keywords absent from the abstract
	...
	8. Disagreement with any link between syndromes and conservation of interaction partners
		...
		\textbf{R:} As the Referee notes, we do address the possibility of convergence later in our introduction. We do not make any claim as to whether the putative pollination syndromes actually reflect plants with common sets of pollinators (the evidence we are aware of is rather mixed). Since the Referee appears to object to this very common idea in the literature, and because syndromes do not form a major part of our analysis, we have removed it as a keyword and rephrased our introduction. Our manuscript no longer references pollination syndromes. Hopefully this will avoid other readers becoming distracted by the idea of syndromes.
	...
	10. Criticisism of methodological changes required by previous reviewers
		...
		\textbf{R:} We restricted our selection of herbivory networks in response to a previous round of review. Without more details on why (or if) the Referee believes that the herbivory networks we removed should be included, we have opted to continue following that earlier feedback. We would have appreciated specific examples for additional herbivory networks, but since the Referee does not provide any specifics there is little we can do to address this comment.
	...
	12. Recommendation to continue using the same methodology
	...
	18. Critique of manuscript structure 
	...
	19. Incorrect claim that beginning of results is inaccurate
	...
	20. Definition of community composition unclear
	...
	24. Confusion about line 328

		\begin{refquote}
			Line  328: "...increasing phylogenetic distance in 12...", what is 12? the number of families?
		\end{refquote}

		\textbf{R:} The number 12 referred to the number of families. We have revised this line to make this more explicit.
	...
	25. Complaints about table numbering and figure referencing
	...
	26. Request for more evolution content
	...
	27. Objection to the word "broad"
		...
		\textbf{Nick:} I think they are suggesting that our evidence was only in 55(6?)\% of our plant-pollinator networks and not in our other network types, thus ''almost half ''. However I agree that our results still show broad support.

		\textbf{R:} We note that 56\% is greater than half of those networks, and that when all networks were considered together we also observed the same trends. We contend that these two facts do represent broad support for our hypothesis. It is not, of course, universal support. That is why we A) did not claim universal support and B) provide the numbers of networks. Readers are thereby armed with the tools to agree or disagree with our choice of words. In an effort to be as unbiased as possible while discussing our results, we have changed this line to state ''We found \emph{general} support...''. We hope that this is clearer.


		With the unbalanced sample sizes of pollination and herbivory networks, we are reluctant to make much of the difference between 56\% of pollination networks and 64\% of herbivory networks. We consider that the important point here is that networks are more likely than not to show decreasing niche overlap with increasing phylogenetic distance. 


	28. Question about subsetting herbivory networks or perhaps objection to the claim that oligotrophy is common
	...
	29. Claims that congeneric species are not closely related
		...
		\textbf{Nick:} This seems like a rather unclear argument from the reviewer. To state that congeneric species can not be closely related is basically arguing that being closely related taxonomically does not mean that those species are closely related phylogenetically. However, I agree that if they are going to make a statement that powerful, they should provide references and a very clear rationale to make the requested change. Can we find a reference where someone else talks about taxonomic and phylogenetic relationships as we have? Just as a justification tool more than anything?

		\textbf{R:} We are confused by this comment, is the Referee suggesting that plants in the same genus are less closely-related than plants in different families? This comment is surprising given the Referee's insistence that we should expect similar interaction partners within a family. We agree that taxonomy is only a proxy for phylogeny, but we are not aware of any evidence that it is a poor proxy. We note that across seven other reviews, no other Referee has commented on this. Based on this lack of objection and similar language throughout the literature, we have opted not to change this line.
	...
	30. Another demand to discuss intimacy [[possibly addressable, since we did add a tiny reference in intro]]
		...
		\textbf{R:} We do not have any data on interaction intimacy, so we cannot say how this would influence our results. [[I'll work on adding some kind of ref to this Hembry thing.]]
