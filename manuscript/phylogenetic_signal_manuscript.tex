...
\section*{Results}
  ...
  \subsection*{Within-network conservation of niche overlap} 
    ...
    permuted networks, there was no relationship, which speaks to the robustness of our methodology (\emph{Supporting 
    information 2}).
  ...
  % Regression of all families at once with RE's for network
    ...
    a further 34 plant families. Finally, the overlap between pairs of \emph{Apiaceae} and \emph{Poaceae} increased significantly with increasing phylogenetic distance.
...
\section*{Figures}
  ...
  \begin{figure}[!h]
    ...
    \end{center}
     \caption{Change in log odds of a pair of plants sharing a pollinator or herbivore as phylogenetic distance between the plants increases. For clarity, we show only the 15 plant families for which the slope of the regression of the proportion of shared interaction partners against phylogenetic distance was significant in at least one network type.       Note that the change in log odds for \emph{Asteraceae} in herbivory networks and \emph{Melastomataceae} and \emph{Poaceae} in pollination networks are not significantly different from zero; we present these values only for comparison across network types. All other plant families were well-represented in only one network type.
     ...
     relatedness between these plant families. Error bars represent 95\% confidence
     intervals.}
